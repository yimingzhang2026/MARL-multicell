...
M = 3
N = 8
drs = [0.3, 0.4, 0.5]  # Different values of dr
methods = ['gms','separate_nn', 'shared_nn', 'qcsma', 'benchmark']
...
for dr in drs:
    ...
    method_paths = {
        'separate_nn': 'separate_nn_' + data_file_path + '.npz',
        'shared_nn': 'shared_nn_' + data_file_path + '.npz',
        'gms': 'gms_' + data_file_path + '.npz',
        'qcsma': 'qcsma_' + data_file_path + '.npz',
        'benchmark': 'benchmark_' + data_file_path + '.npz'
    }
...
bar_width = 0.15
opacity = 0.8
colors = ['brown', 'purple','darkgreen', 'steelblue', 'orange']
labels = ['GMS','separate_nn','shared_nn', 'QCSMA', 'LLQ']
hatches = ['o','/-', '+', '\\', 'x']
y_lim = 8
# Set x-axis labels
...
for i, method in enumerate(methods):
    ...
    rects = ax.bar(index + i * bar_width, delays, bar_width, alpha=opacity, color=colors[i], label=labels[i], hatch=hatches[i])
