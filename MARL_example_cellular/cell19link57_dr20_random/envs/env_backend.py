...
#         plt.text(TX_loc[0,i],TX_loc[1,i], 'AP{}'.format(i), fontsize=12)
...
#         plt.text(RX_loc[0,i],RX_loc[1,i], 'D{}'.format(i), fontsize=12)
#     for i in range(1,N): 
...
#     #plt.savefig('./cellular_deployment.eps', format='eps',dpi=1600)
#     plt.show()    
#%%
...
def generate_deployment_hexa(R, K, N, seed):
    ...
    generated_APs += 1
    # Generate fixed AP location using random seed
    random_state = np.random.RandomState(seed)
    ...
    while generated_APs < K:
        for j in range(6):
            ...
            was_before = False
            for inner_loop in range(generated_APs):
                if (abs(tmp_xloc - TX_loc[0, inner_loop]) < R * 1e-2 and abs(tmp_yloc - TX_loc[1, inner_loop]) < R * 1e-2):
                    was_before = True
                    break
            if not was_before:
                TX_loc[0, generated_APs] = tmp_xloc
